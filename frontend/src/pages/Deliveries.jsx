  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 sm:mb-6 gap-3">
        <h1 className="text-xl sm:text-2xl font-bold text-orange-600">Outgoing Deliveries</h1>
        <button onClick={() => navigate('/deliveries/new')}
          className="bg-orange-600 text-white px-4 py-2 rounded hover:bg-orange-700 w-full sm:w-auto text-sm sm:text-base"
        ...
        <div className="overflow-x-auto">
          ...
          <tbody>
            {filteredMoves.length === 0 ? (
              <tr>
                <td colSpan="8" className="p-4 text-center text-gray-500 text-xs sm:text-sm">No deliveries found</td>
              </tr>
            ) : (
              filteredMoves.map(move => (
                <tr key={move._id} className="border-b hover:bg-gray-50">
                  ...
                  <td className="p-2 sm:p-4 font-medium text-xs sm:text-sm">{move.productId?.name}</td>
                  <td className="p-2 sm:p-4 font-bold text-red-500 text-xs sm:text-sm">-{move.quantity}</td>
                    ...
                    {move.status === 'draft' && (
                      isManager ? (
                        <button 
                          ...
                          className="bg-green-600 text-white px-2 sm:px-4 py-1 sm:py-2 rounded hover:bg-green-700 text-xs sm:text-sm"
                        >Validate</button>
                      ) : (
                        <span className="text-gray-500 italic text-xs">Manager Only</span>
                      )
                    )}
                    {move.status === 'done' && (
                  ...
                  <td className="cursor-pointer text-blue-600 text-xs sm:text-sm p-2 sm:p-4" onClick={() => navigate(`/deliveries/${move._id}`)}>View</td>
                </tr>
              ))
            )}
          </tbody>
